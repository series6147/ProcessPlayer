...
{
    public class ToRdMSSQL : ProcessContent
    {
            ...
            return Allowed()
                ? await Task<DataExchangeObject[]>.Factory.StartNew(() =>
                {
                    var connection = string.IsNullOrEmpty(ConnectionString)
                        ? new SqlConnection(string.Format("Server={0};Initial Catalog={1};User ID={2};Password={3};Integrated Security={4};", Server, Database, UserID, Password, IntegratedSecurity))
                    ...
                    try
                    {
                        ...
                        using (var command = new SqlCommand(Definition, connection) { CommandTimeout = 0, CommandType = CommandType })
                        {
                            if (Parameters != null)
                                command.Parameters.AddRange(Parameters.Select(kvp => new SqlParameter(kvp.Key, Vars[kvp.Value])).ToArray());
